...
/**
 * setadapter 最后设置，其他的先设置
 * pullRefreshEnabled 在addheaeder 之前设置
 */
...
public class XRecyclerView extends RecyclerView {
    ...
    /**
     * 默认没有下拉刷新
     * 可以兼容swiperefreshview，
     * 需要的话可以设置为true，设置为true，外层有swiperefreshview嵌套时，会同时触发
     */
    ...
    public void addHeaderView(View view) {
        if (pullRefreshEnabled && mHeaderViews.size() == 0) {
            ArrowRefreshHeader refreshHeader = new ArrowRefreshHeader(mContext);
            mHeaderViews.add(0, refreshHeader);
            mRefreshHeader = refreshHeader;
            mRefreshHeader.setProgressStyle(mRefreshProgressStyle);
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            boolean rstate = true;
            if (mRefreshHeader != null) {
                rstate = mRefreshHeader.getState() < ArrowRefreshHeader.STATE_REFRESHING;
            }
            if (layoutManager.getChildCount() > 0
                    && lastVisibleItemPosition >= layoutManager.getItemCount() - 1 && layoutManager.getItemCount() > layoutManager.getChildCount() && !isnomore && rstate) {
                View footView = mFootViews.get(0);
                isLoadingData = true;
                if (footView instanceof LoadingMoreFooter) {
